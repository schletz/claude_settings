...
def find_cut_points(path: str, duration: float, target: float,
    ...
    print(f"WARNUNG: Auch bei {SILENCE_STEPS[-1]:.0f} dB bleibt der laengste Block bei "
          f"{longest_block(best) / 60:.1f} Minuten. Bei einem Abbruch die Bloecke von "
          "Hand schneiden - siehe SKILL.md, Schritt 4.", file=sys.stderr)
    return best


def block_signature(start: float, end: float, lang: str | None, model: str) -> str:
    """Identify a cached block by everything that would change its decoding."""
    return f"{start:.3f}|{end:.3f}|{lang}|{model}"


def load_block(path: str, signature: str) -> dict | None:
    """Return a cached block if it exists and was decoded under the same conditions."""
...
def decode_block(asr, path: str, start: float, end: float, kwargs: dict) -> dict:
    """Decode one time slice and return its text plus word times in absolute seconds."""
    length = end - start
    ...
    for chunk in result.get("chunks", []):
        ...
        words.append({"text": chunk["text"],
                      "start": min(begin, length) + start,
                      "end": min(finish, length) + start})
    return {"text": result["text"].strip(), "words": words}


def as_chunks(words: list[dict]) -> list[dict]:
    """Convert stored word records back into the pipeline's chunk shape."""
    return [{"text": w["text"], "timestamp": (w["start"], w["end"])} for w in words]


def report_chunking(path: str, blocks: int, duration: float, max_chunk: float) -> None:
    """Announce the block count and warn when single blocks stay dangerously long."""
    ...
    if blocks < duration // max_chunk:
        print(f"WARNUNG: {blocks} Bloecke fuer {duration / 60:.0f} Minuten - einzelne "
              ...
              "hart abbrechen lassen. Mit --silence-db nachhelfen.", file=sys.stderr)


def decode_variant(asr, path: str, lang: str | None, duration: float, max_chunk: float,
                   silence_db: float | None = None, model: str = "") -> tuple[str, list[dict]]:
    """Decode one variant end to end, chunking the file if it is too long.

    The long-form pass with word timestamps dies on very long inputs: a hard process
    abort, no traceback, no exit code worth reading. Chunking at pauses keeps every
    decode small enough; the timestamps are shifted back to absolute time.

    Every finished block is written to <variant>_blocks/ before the next one starts. The
    abort can still happen - it killed a run after four of five blocks - but a rerun then
    replays the cache and resumes at the block that died, instead of redoing everything.
    The cache key covers the block bounds, the language and the model, so changing any of
    them re-decodes rather than silently reusing a stale reading.
    """
    ...
    report_chunking(path, len(cuts) - 1, duration, max_chunk)

    cache_dir = os.path.splitext(path)[0] + "_blocks"
    os.makedirs(cache_dir, exist_ok=True)
    texts: list[str] = []
    collected: list[dict] = []
    for index, (start, end) in enumerate(zip(cuts, cuts[1:])):
        cache_path = os.path.join(cache_dir, f"block_{index:02d}.json")
        signature = block_signature(start, end, lang, model)
        block = load_block(cache_path, signature)
        origin = " (aus Cache)"
        if block is None:
            block = decode_block(asr, path, start, end, kwargs)
            with open(cache_path, "w", encoding="utf-8") as fh:
                json.dump({"signature": signature, **block}, fh, ensure_ascii=False)
            origin = ""
        texts.append(block["text"])
        collected.extend(as_chunks(block["words"]))
        print(f"  {stamp(start)}-{stamp(end)}: {len(block['words'])} Einheiten{origin}",
              file=sys.stderr)
...
def mode_full(asr, workdir, variants, lang, out, json_out, clip, words,
              duration=0.0, max_chunk=0.0, silence_db=None, model="") -> None:
...
def main() -> None:
    ...
    parser.add_argument("--max-chunk", type=float, default=0.0,
                        help="full mode: split files longer than this into pause-aligned "
                             "chunks (seconds, 0 = off). Required above ~20 minutes; 240 "
                             "is the value that held where 300 aborted.")
    ...
    # Observed abort at 26 minutes, so the old 30-minute threshold was too generous.
    if args.mode == "full" and not args.max_chunk and duration > 1200:
        print(f"WARNUNG: {duration / 60:.0f} Minuten ohne --max-chunk. Lange Dateien "
              "brechen im Langform-Durchlauf hart ab - empfohlen: --max-chunk 240.",
              file=sys.stderr)
